...
//*********************************************************************
// 说明：
// 本文件是 SolidWorks 插件（Add-in）的核心入口类。
// 插件是 SOLIDWORKS 扩展机制，允许第三方代码与 SOLIDWORKS 集成。
//
// 插件架构：
// 1. ISwAddInEx 接口：定义插件的基本功能
// 2. SwAddInEx 基类：提供插件的默认实现
// 3. COM 注册：通过 COM 互操作注册到 SOLIDWORKS
//
// 插件核心功能：
// 1. 命令管理 - 创建自定义命令和工具栏
// 2. 属性页 - 创建 PropertyManager Page 与用户交互
// 3. 任务窗格 - 创建任务窗格工具面板
// 4. 弹出窗口 - 创建模态/非模态对话框
// 5. 文档标签 - 创建文档关联的面板
//
// 生命周期：
// 1. Connect - 插件加载时调用，初始化资源
// 2. Detach - 插件卸载时调用，释放资源
//*********************************************************************
